...
window.showToast=function(opts){
  var container=g('wb-toast-container');
  if(!container){
    container=document.createElement('div');
    container.id='wb-toast-container';
    container.style.cssText='position:fixed;bottom:24px;right:24px;z-index:400000;display:flex;flex-direction:column;gap:10px;pointer-events:none;max-width:340px';
...
function updBell(){
  ...
  var bell=g('wb-bellt');
  if(u>0){
    // Ring the bell — remove and re-add class to restart animation
    bell.classList.remove('ringing');
    void bell.offsetWidth; // force reflow
    bell.classList.add('ringing');
    // Keep ringing every 4s while there are unread notifs
    clearInterval(window._bellTimer);
    window._bellTimer=setInterval(function(){
      if(!nh.filter(function(n){return !n.read;}).length){
        clearInterval(window._bellTimer);
        return;
      }
      bell.classList.remove('ringing');
