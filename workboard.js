...
function av(uid2,s){
  ...
  if(!u) return '<div style="width:'+s+'px;height:'+s+'px;background:#eee;border-radius:50%;flex-shrink:0"></div>';
  return '<div class="av" title="'+u.name+'" style="width:'+s+'px;height:'+s+'px;background:'+u.color+';color:'+u.tc+';font-size:'+(s<24?9:11)+'px;flex-shrink:0">'+u.i+'</div>';
}
...
function buildHTML(){
  ...
  '<div id="wb-login" style="display:none">'+
    '<div class="wb-login-card">'+
      '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">'+
        '<div style="width:30px;height:30px;border-radius:8px;background:#F7931E;display:flex;align-items:center;justify-content:center;flex-shrink:0">'+
  ...
  '<div id="wb-app" style="display:none">'+
    '<div id="wb-sb">'+
      ...
      '<div class="sb-item" id="sbn-boards" onclick="setTab(\'boards\')">'+
        '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg> Tasks'+
      '</div>'+
      '<div class="sb-item" id="sbn-brand" onclick="setTab(\'brand\')">'+
        '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M6 20v-2a6 6 0 0 1 12 0v2"/></svg> Brand Kit'+
      '</div>'+
      '<div class="sb-item" id="sbn-trash" onclick="setTab(\'trash\')">'+
        '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg> Trash'+
      '</div>'+
      ...
      '<div id="wb-bl"></div>'+
      '<div class="sb-item" id="wb-nbb" style="color:#9CA3AF">'+
        '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> New task board'+
      '</div>'+
    ...
    '<div id="wb-main">'+
      '<div id="wb-topbar">'+
        '<button class="btn" style="padding:5px 8px" id="wb-tsb">'+
        ...
        '<div style="position:relative" id="wb-bell-btn">'+
          '<button class="btn" style="padding:5px 10px" id="wb-bellt">'+
  ...
  '<div id="wb-toast-container" style="position:fixed;bottom:24px;right:24px;z-index:400000;display:flex;flex-direction:column;gap:10px;pointer-events:none;max-width:340px"></div>'+
    ...
    '<div style="padding:16px 18px 18px">'+
      '<div style="display:flex;align-items:flex-start;gap:14px">'+
        '<div style="width:44px;height:44px;border-radius:50%;background:#D6F5E8;display:flex;align-items:center;justify-content:center;flex-shrink:0">'+
          '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1D7A4E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>'+
        '</div>'+
        '<div style="flex:1">'+
          '<div style="font-size:15px;font-weight:700;margin-bottom:4px;color:#1A2228">Task completed!</div>'+
          '<div style="font-size:13px;color:#3D4F5C;line-height:1.6" id="wb-nm"></div>'+
        '</div>'+
        '<button onclick="wbHP()" style="background:none;border:none;font-size:18px;cursor:pointer;color:#9CA3AF;line-height:1">&#215;</button>'+
      '</div>'+
      '<div style="margin-top:14px;height:3px;border-radius:2px;background:#F2F3F5;overflow:hidden">'+
        '<div id="wb-npb" style="height:100%;background:#F7931E;border-radius:2px"></div>'+
      '</div>'+
    '</div>'+
  '</div>'+
  '<div id="wb-np">'+
    '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid #E2E5E8">'+
      '<span style="font-size:14px;font-weight:700;color:#1A2228">Notifications</span>'+
      '<button id="wb-npc" style="background:none;border:none;font-size:12px;color:#9CA3AF;cursor:pointer;font-family:inherit">Clear all</button>'+
    '</div>'+
  ...
  '<div id="wb-cpw-modal" style="display:none;position:fixed;inset:0;background:rgba(26,34,40,.55);z-index:300000;align-items:center;justify-content:center">'+
    '<div style="background:#fff;border-radius:16px;padding:32px;width:360px;max-width:92vw;box-shadow:0 20px 60px rgba(0,0,0,.2)">'+
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">'+
        '<span style="font-size:15px;font-weight:700;color:#1A2228">Change Password</span>'+
        '<button onclick="wbCloseCPW()" style="background:none;border:none;font-size:22px;cursor:pointer;color:#9CA3AF">&#215;</button>'+
      '</div>'+
      ...
      '<div style="display:flex;gap:8px">'+
        '<button onclick="wbCloseCPW()" style="flex:1;padding:10px;border-radius:8px;border:1px solid #E2E5E8;background:#fff;font-size:13px;cursor:pointer;font-family:inherit;color:#1A2228">Cancel</button>'+
        '<button onclick="wbSubmitCPW()" style="flex:1;padding:10px;border-radius:8px;border:none;background:#F7931E;color:#fff;font-size:13px;cursor:pointer;font-weight:600;font-family:inherit">Save password</button>'+
      '</div>'+
    '</div>'+
  '</div>';
}
...
function mkModal(id,tid,title,body,left,right){
  return '<div class="wb-mo" id="'+id+'"><div class="wb-mb">'+
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">'+
      '<span style="font-size:15px;font-weight:700;color:#1A2228" id="'+tid+'">'+title+'</span>'+
      '<button id="'+id+'c" style="background:none;border:none;font-size:22px;cursor:pointer;color:#9CA3AF">&#215;</button>'+
    '</div>'+
    body+
...
document.addEventListener('keydown',function(e){
  if(e.key==='Enter'&&g('wb-pw-screen')&&g('wb-pw-screen').style.display==='flex') wbPWSubmit();
});
...
function wire(){
  ...
  g('wb-npc').onclick=function(){
    ...
    nh=[]; updBell(); rNP(); g('wb-np').classList.remove('open');
  };
...
window.setTab=function(tab){
  ...
  if(tab==='boards'&&getB()){ g('wb-bcd').style.display='block'; g('wb-bcd').style.background=getB().color; }
  else g('wb-bcd').style.display='none';
  rC();
};
...
function rSB(){
  ...
  g('wb-sbu').innerHTML=
    ...
    '<button onclick="wbOpenChangePW()" title="Change password" style="background:none;border:none;cursor:pointer;color:#9CA3AF;padding:2px 4px;display:flex;align-items:center">'+
      '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>'+
    '</button>'+
    '<button onclick="wbLO()" title="Sign out" style="background:none;border:none;cursor:pointer;color:#9CA3AF;padding:2px 4px;display:flex;align-items:center">'+
  ...
  for(var i=0;i<Math.min(mt.length,5);i++){
    var t=mt[i];
    mh+='<div style="display:flex;align-items:center;gap:8px;padding:5px 4px;font-size:12px;border-radius:6px;cursor:pointer;color:#1A2228" onclick="wbET(\''+t.id+'\')">'+
      '<div class="pdot pd-'+t.priority+'"></div>'+
      '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1">'+t.name+'</span>'+
      '<button class="qdone" onclick="event.stopPropagation();wbQD(\''+t.id+'\')">&#10003;</button>'+
    '</div>';
  }
  ...
  for(var i=0;i<boards.length;i++){
    ...
    bh+='<div class="sb-item'+(abid===bd.id&&activeTab==='boards'?' active':'')+'" onclick="wbSB(\''+bd.id+'\')">'+
      '<div style="width:10px;height:10px;border-radius:3px;flex-shrink:0;background:'+bd.color+'"></div>'+
      '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1">'+bd.name+'</span>'+
      '<span style="font-size:11px;margin-left:auto">'+cnt+'</span>'+
    '</div>';
  }
...
function rC(){
  ...
  if(!b){
    c.innerHTML='<div class="wb-empty">'+
      '<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#C8CDD1" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>'+
      '<div style="font-size:16px;font-weight:600;color:#1A2228">No task boards yet</div>'+
      '<div style="font-size:13px;color:#6B7A84">Create your first board to get started</div>'+
...
function rTbl(board,c){
  ...
  for(var gi=0;gi<board.groups.length;gi++){
    ...
    h+='<div style="display:flex;align-items:center;gap:8px;padding:4px 6px;margin-bottom:8px">';
    ...
    h+='<span style="font-size:14px;font-weight:700;color:#1A2228">'+gr.name+'</span>';
    h+='<span style="font-size:12px;color:#9CA3AF;margin-left:2px;font-weight:500">'+disp.length+'</span>';
    h+='<div style="margin-left:auto;display:flex;gap:6px">'+
      '<button class="btn" style="padding:3px 10px;font-size:12px" onclick="oAT(\''+gr.id+'\')">+ Task</button>'+
      '<button class="btn" style="padding:3px 8px;font-size:12px" onclick="wbEG(\''+gr.id+'\')">&#8943;</button>'+
    '</div>';
    h+='</div>';
    if(!isc){
      h+='<div style="background:#fff;border:1px solid #E2E5E8;border-radius:10px;overflow:hidden;margin-bottom:4px">';
      ...
      if(!hF) h+='<div style="padding:8px 14px;font-size:12px;color:#9CA3AF;cursor:pointer;border-top:1px solid #F2F3F5;transition:background .1s" onmouseover="this.style.background=\'#F7F8FA\'" onmouseout="this.style.background=\'\'" onclick="oAT(\''+gr.id+'\')">+ Add task</div>';
      h+='</div>';
    }
    h+='</div>';
  }
  ...
  for(var si=0;si<ST.length;si++){
    ...
    h+='<div class="kan-col">'+
      '<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">'+
        ...
        '<span style="font-size:13px;font-weight:700;color:#38444E">'+sc.l+'</span>'+
        '<span style="font-size:12px;color:#9CA3AF;margin-left:auto;font-weight:500">'+tasks.length+'</span>'+
      '</div>';
    for(var ti=0;ti<tasks.length;ti++){
      var t=tasks[ti].item, gc=tasks[ti].gc;
      var im=t.ownerId===cu.id&&t.status!=='done';
      var s=ST[0]; for(var si2=0;si2<ST.length;si2++) if(ST[si2].v===t.status){ s=ST[si2]; break; }
      h+='<div class="kan-card'+(im?' mine':'')+'" style="border-left:3px solid '+gc+'" onclick="wbET(\''+t.id+'\')">'+
        ...
        '<div style="display:flex;align-items:center;gap:6px">'+
          '<div class="pdot pd-'+t.priority+'"></div>'+
          (t.due?'<span style="font-size:11px;color:#6B7A84">'+t.due+'</span>':'')+
...
function uFil(){
  for(var i=0;i<PR.length;i++){
    ...
    e.style.background=a?'#F7931E':'#fff';
    e.style.color=a?'#fff':'#6B7A84';
    e.style.borderColor=a?'#F7931E':'#E2E5E8';
  }
  var sm={
...
// ── TASK MODAL ────────────────────────────────────────────────
function pSS(v,disabled){
  var h='';
  for(var i=0;i<ST.length;i++) h+='<option value="'+ST[i].v+'"'+(ST[i].v===v?' selected':'')+'>'+ST[i].l+'</option>';
  var sel=g('wb-ts'); sel.innerHTML=h; sel.disabled=!!disabled; sel.style.opacity=disabled?'.5':'1';
}
function pPS(v){
  var h='';
  for(var i=0;i<PR.length;i++) h+='<option value="'+PR[i].v+'"'+(PR[i].v===v?' selected':'')+'>'+PR[i].l+'</option>';
  g('wb-tp').innerHTML=h;
}
function rAL(lockOwner){
  ...
  for(var i=0;i<TM.length;i++){
    var u=TM[i];
    if(lockOwner){
      h+='<div class="aitem'+(u.id===sO?' sel':'')+'" style="--c:'+u.color+';opacity:'+(u.id===sO?1:.4)+';cursor:default">'+av(u.id,28)+'<span style="font-size:13px;font-weight:'+(u.id===sO?600:400)+';color:#1A2228">'+u.name+'</span>'+(u.id===sO?'<span style="margin-left:auto;font-size:13px;color:'+u.color+'">&#10003;</span>':'')+'</div>';
    } else {
      h+='<div class="aitem'+(u.id===sO?' sel':'')+'" style="--c:'+u.color+'" onclick="wbSO(\''+u.id+'\')">'+av(u.id,28)+'<span style="font-size:13px;font-weight:'+(u.id===sO?600:400)+';color:#1A2228">'+u.name+'</span>'+(u.id===sO?'<span style="margin-left:auto;font-size:13px;color:'+u.color+'">&#10003;</span>':'')+'</div>';
    }
  }
  g('wb-al').innerHTML=h;
}
...
function rBrand(c){
  ...
  for(var i=0;i<bkeys.length;i++){
    var bk2=BRANDS[bkeys[i]], isA=bkeys[i]===activeBrand;
    tabs+='<button onclick="wbBrandTab(\''+bkeys[i]+'\')" style="padding:8px 20px;border-radius:8px;border:'+(isA?'none':'1px solid #E2E5E8')+';background:'+(isA?'#F7931E':'#fff')+';color:'+(isA?'#fff':'#6B7A84')+';font-size:13px;font-weight:'+(isA?700:500)+';cursor:pointer;font-family:inherit">'+bk2.name+'</button>';
  }
  tabs+='</div>';

  var logoSrc=bk.logoData||bk.logo;
  var logoSec='<div style="background:#fff;border:1px solid #E2E5E8;border-radius:12px;padding:24px;margin-bottom:20px">'+
    '<div style="font-size:11px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:.08em;margin-bottom:16px">Logo</div>'+
    '<div style="display:flex;align-items:center;gap:24px;flex-wrap:wrap">'+
      '<div id="wb-logo-preview-'+activeBrand+'" style="background:#F7F8FA;border:1px solid #E2E5E8;border-radius:10px;padding:24px 32px;display:flex;align-items:center;justify-content:center;min-width:200px;min-height:80px">'+
        (logoSrc?'<img src="'+logoSrc+'" style="max-height:60px;max-width:200px;object-fit:contain"/>':'<span style="font-size:12px;color:#9CA3AF">No logo uploaded</span>')+
      '</div>'+
      '<div style="display:flex;flex-direction:column;gap:8px">'+
        '<label style="display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border-radius:8px;border:1px solid #E2E5E8;background:#F2F3F5;color:#38444E;font-size:13px;cursor:pointer;font-family:inherit;font-weight:500">'+
          '&#11015; Upload Logo<input type="file" accept="image/*" style="display:none" onchange="wbUploadLogo(this,\''+activeBrand+'\')"/>'+
        '</label>'+
        (logoSrc?'<a href="'+logoSrc+'" download target="_blank" style="display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border-radius:8px;border:1px solid #E2E5E8;background:#fff;color:#1A2228;font-size:13px;text-decoration:none;font-family:inherit">&#11015; Download Logo</a>':'')+
      '</div>'+
    '</div>'+
  '</div>';

  var colorSec='<div style="background:#fff;border:1px solid #E2E5E8;border-radius:12px;padding:24px;margin-bottom:20px">'+
    '<div style="font-size:11px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:.08em;margin-bottom:16px">Color Palette</div>'+
  ...
  for(var i=0;i<bk.colors.length;i++){
    ...
    colorSec+='<div style="width:160px">'+
      '<div onclick="wbCopyColor(\''+clr.hex+'\')" title="Click to copy hex" style="width:160px;height:80px;border-radius:8px;background:'+clr.hex+';border:1px solid rgba(0,0,0,.08);cursor:pointer;display:flex;align-items:flex-end;padding:8px;transition:transform .1s" onmouseover="this.style.transform=\'scale(1.03)\'" onmouseout="this.style.transform=\'scale(1)\'">'+
        '<span style="font-size:11px;font-weight:700;color:'+(isDark?'rgba(255,255,255,.85)':'rgba(0,0,0,.55)')+'">'+clr.hex+'</span>'+
      '</div>'+
      '<div style="margin-top:8px">'+
        '<div style="font-size:13px;font-weight:700;color:#1A2228">'+clr.name+'</div>'+
        (clr.cmyk?'<div style="font-size:11px;color:#6B7A84;margin-top:2px">'+clr.cmyk+'</div>':'')+
        (clr.rgb?'<div style="font-size:11px;color:#9CA3AF">'+clr.rgb+'</div>':'')+
      '</div>'+
    '</div>';
  }
  colorSec+='</div></div>';

  var fontSec='<div style="background:#fff;border:1px solid #E2E5E8;border-radius:12px;padding:24px;margin-bottom:20px">'+
    '<div style="font-size:11px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:.08em;margin-bottom:16px">Typography</div>'+
    '<div style="display:flex;flex-direction:column;gap:12px">';
  for(var i=0;i<bk.fonts.length;i++){
    var f=bk.fonts[i];
    fontSec+='<div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border:1px solid #E2E5E8;border-radius:10px;flex-wrap:wrap;gap:12px">'+
      '<div>'+
        '<div style="font-size:20px;font-weight:700;margin-bottom:6px;color:#1A2228">'+f.name+'</div>'+
        '<div style="font-size:12px;color:#38444E;margin-bottom:2px"><strong>Role:</strong> '+f.role+'</div>'+
        '<div style="font-size:12px;color:#6B7A84;margin-bottom:2px"><strong>Weights:</strong> '+f.weights+'</div>'+
        (f.designers?'<div style="font-size:12px;color:#9CA3AF"><strong>By:</strong> '+f.designers+'</div>':'')+
      '</div>'+
      '<a href="'+f.url+'" target="_blank" style="display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border-radius:8px;border:1px solid #E2E5E8;background:#fff;color:#1A2228;font-size:12px;text-decoration:none;font-family:inherit;white-space:nowrap;font-weight:500">&#128279; View Font</a>'+
    '</div>';
  }
  fontSec+='</div></div>';

  var storedAssets=bk.uploadedAssets||[];
  var assetSec='<div style="background:#fff;border:1px solid #E2E5E8;border-radius:12px;padding:24px;margin-bottom:20px">'+
    '<div style="font-size:11px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:.08em;margin-bottom:16px">Assets</div>'+
    '<div id="wb-assets-list-'+activeBrand+'" style="display:flex;flex-wrap:wrap;gap:12px;margin-bottom:14px">';
  if(!storedAssets.length) assetSec+='<div style="font-size:13px;color:#9CA3AF;padding:4px 0">No assets uploaded yet</div>';
  for(var i=0;i<storedAssets.length;i++){
    var a=storedAssets[i];
    assetSec+='<div style="display:flex;flex-direction:column;align-items:center;gap:6px;padding:12px;border:1px solid #E2E5E8;border-radius:10px;background:#F7F8FA;width:120px">'+
      '<img src="'+a.data+'" style="max-width:90px;max-height:60px;object-fit:contain;border-radius:4px"/>'+
      '<span style="font-size:11px;color:#3D4F5C;text-align:center;word-break:break-word;max-width:100px;font-weight:500">'+a.name+'</span>'+
      '<div style="display:flex;gap:4px">'+
        '<a href="'+a.data+'" download="'+a.name+'" style="font-size:11px;color:#F7931E;text-decoration:none;padding:2px 8px;border:1px solid #F7931E;border-radius:4px;font-weight:600">&#11015;</a>'+
        '<button onclick="wbDeleteAsset(\''+activeBrand+'\','+i+')" style="font-size:11px;color:#B03020;background:none;border:1px solid #e0b0aa;border-radius:4px;padding:2px 6px;cursor:pointer;font-family:inherit">&#215;</button>'+
      '</div>'+
    '</div>';
  }
  assetSec+='</div>'+
    '<label style="display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border-radius:8px;border:1px solid #E2E5E8;background:#F2F3F5;color:#38444E;font-size:13px;cursor:pointer;font-family:inherit;font-weight:500">'+
      '+ Upload Asset<input type="file" accept="image/*,application/pdf,.svg,.eps,.ai,.zip" multiple style="display:none" onchange="wbUploadAsset(this,\''+activeBrand+'\')"/>'+
    '</label>'+
  '</div>';
  ...
  if(!g('wb-color-toast')){
    ...
    t.id='wb-color-toast';
    t.style.cssText='position:fixed;bottom:24px;left:50%;transform:translateX(-50%) translateY(80px);transition:transform .3s;background:#38444E;color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;z-index:300000;pointer-events:none;font-weight:500';
...
function rTrash(c){
  if(!trash.length){
    c.innerHTML='<div class="wb-empty">'+
      '<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#C8CDD1" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>'+
      '<div style="font-size:16px;font-weight:600;color:#1A2228">Trash is empty</div>'+
      '<div style="font-size:13px;color:#6B7A84">Deleted tasks appear here for 30 days</div>'+
    '</div>';
    return;
  }
  var h='<div style="padding:4px 0">'+
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">'+
      '<span style="font-size:13px;color:#6B7A84">'+trash.length+' item'+(trash.length!==1?'s':'')+' &nbsp;·&nbsp; Auto-deleted after 30 days</span>'+
      '<button onclick="wbEmptyTrash()" class="btn btnd" style="font-size:12px">Empty trash</button>'+
    '</div>'+
    '<div style="background:#fff;border:1px solid #E2E5E8;border-radius:10px;overflow:hidden">';
  for(var i=0;i<trash.length;i++){
    ...
    h+='<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;border-bottom:1px solid #F2F3F5">'+
      '<div style="flex:1;min-width:0">'+
        '<div style="font-size:13px;color:#9CA3AF;text-decoration:line-through;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+it.name+'</div>'+
        '<div style="font-size:11px;color:#C8CDD1;margin-top:2px">'+daysLeft+' day'+(daysLeft!==1?'s':'')+' left</div>'+
      '</div>'+
      '<span class="pill '+s.p+'">'+s.l+'</span>'+
...
window.wbRestore=function(itemId){
  ...
  b.groups[0].items.unshift({id:item.id,name:item.name,status:item.status,priority:item.priority,ownerId:item.ownerId,assignedBy:item.assignedBy,due:item.due,notes:item.notes});
  var nt=[];
  for(var i=0;i<trash.length;i++) if(trash[i]&&trash[i].id!==itemId) nt.push(trash[i]);
  trash=nt;
      ...
      reader.onload=function(e){
        ...
        if(done===toRead){
          db.ref('workboard/brandAssets/'+brandKey+'/assets').set(bk.uploadedAssets,function(){
            wbBrandToast(toRead+' asset'+(toRead>1?'s':'')+' uploaded!'); rC();
          });
        }
      };
      reader.readAsDataURL(file);
...
window.wbDeleteAsset=function(brandKey,index){
  var bk=BRANDS[brandKey]; if(!bk.uploadedAssets) return;
  bk.uploadedAssets.splice(index,1);
  db.ref('workboard/brandAssets/'+brandKey+'/assets').set(bk.uploadedAssets,function(){ wbBrandToast('Asset deleted.'); rC(); });
};
...
window.wbOpenChangePW=function(){
  g('wb-cpw-cur').value=''; g('wb-cpw-new').value=''; g('wb-cpw-cfm').value='';
  g('wb-cpw-err').textContent=''; g('wb-cpw-ok').textContent='';
...
window.wbSubmitCPW=function(){
  var cur=g('wb-cpw-cur').value, nw=g('wb-cpw-new').value, cfm=g('wb-cpw-cfm').value;
  g('wb-cpw-err').textContent=''; g('wb-cpw-ok').textContent='';
  ...
  db.ref('workboard/passwords/'+cu.id).set(nw,function(err){
    if(err){ g('wb-cpw-err').textContent='Failed to save. Please try again.'; }
    else{
      PASSWORDS[cu.id]=nw;
...
function start(){
  ...
  var s=loadSession();
  if(s){ doLogin(s); }
  else { g('wb-login').style.display='flex'; rLogin(); }
}
